		...
		1, /*autoConnect*/ 0, /*default priority */ 0 /*default stack size*/ )
{
	...
	if (!pasynInterface) {
		asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
				"%s:%s: %s interface not supported",
				driverName, functionName, asynCommonType);
		return;
	}
	...
	if (!pasynInterface) {
		asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
				"%s:%s: %s interface not supported",
				driverName, functionName, asynOctetType);
		return;
	}
...
void zebra::pollTask() {
	...
	unsigned int time;
	const reg *r;
	unsigned int i, poll=0, sys=0, dosys=1, nfound;
	...
	while (true) {
		...
		while (epicsMessageQueuePending(this->intQId) > 0) {
			...
			if (strcmp(rxBuffer, "PR") == 0) {
				...
				for (int a=0; a<NARRAYS; a++) {
					if (cap>>a & 1) {
						if (sscanf(ptr, "%08X%n", &value, &incr) != 1) {
							asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
									"%s:%s: Bad interrupt on encoder %d\n",
									driverName, functionName, a+1);
							break;
						}
						ptr += incr;
					} else {
				...
				// sanity check
				if (ptr[0] != '\0') {
					epicsStrnEscapedFromRaw(escapedbuff, NBUFF, ptr, strlen(ptr));
					asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
...
asynStatus zebra::getReg(const reg *r, int *value) {
	const char *functionName = "getReg";
	asynStatus status = asynSuccess;
	// Create the transmit buffer
	char txBuffer[NBUFF], escapedbuff[NBUFF], *rxBuffer;
	int txSize, addr;
	// Send a write
	...
	if(status == asynSuccess) {
		if (sscanf(rxBuffer, "R%02X%04X", &addr, value)) {
			...
			epicsStrnEscapedFromRaw(escapedbuff, NBUFF, rxBuffer, strlen(rxBuffer));
			asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
					"%s:%s: Unrecognised message '%s'\n",
					driverName, functionName, escapedbuff);
			status = asynError;
		}
		free(rxBuffer);
