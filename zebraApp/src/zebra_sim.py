...
for line in re.sub(r"/\*(.|[\r\n])*?\*/", "", reg_lookup_str).splitlines():
    if line.strip():
        ...
        name_lookup[int(num, 0)] = name
        reg_lookup[name] = int(num, 0)
...
class zebra(serial_device):
    ...
    def __init__(self):    
        self.i = 0
        self.memory = dict((x, 0) for x in name_lookup)
    ...
    def poll(self):
        if self.armed:            
            ret = "P%08X" % (self.i*50)
            self.i+=1
            bits = self.memory[reg_lookup["PC_BIT_CAP"]]            
            for i in range(16):
                if (bits >> i) & 1:
    ...
    def reply(self, command):
        ...
        if command == "S":
            ...
            if addr in self.memory:
                ...
                ret = ""
                if name_lookup[addr] == "PC_ARM":
                    self.armed = 1
                    ret = "PR\n"
                if name_lookup[addr] == "PC_DISARM":
                    self.armed = 0
                    ret = "PX\n"
